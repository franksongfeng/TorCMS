    ...
    for key in post_cfg.keys():
        ...
        for recent_post in recent_posts:
            ...
            if hist_rec:
                ...
                diff_str = (
                    diff_str
                    + '<h3>CONTENT:{0}</h3>'.format(recent_post.title)
                    + infobox
                    + '</hr>'
                )
            else:
...
def __get_post_review(email_cnt, idx):
    ...
    for key in post_cfg.keys():
        ...
        for recent_post in recent_posts:
            hist_rec = MPostHist.get_last(recent_post.uid)
            editor_name = recent_post.extinfo.get(
                'def_editor_name', recent_post.user_name
            )
            if hist_rec:
                foo_str = (
                    '<tr><td>{0}</td><td>{1}</td>'
...
.diff_sub {background-color:#ffaaaa}
    ...
    email_cnt = (
        email_cnt
        + '<table border=1><tr><td colspan=5><center><h3>文档更新情况<h3></center></td></tr>'
    )
