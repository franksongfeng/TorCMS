...
def prepare_data(filepath):
    ...
    # filepath = './xx_title/1/0100/0111.md'
    ...
    # print(cnts)
    return cnts, [x.strip().split('|')[1] for x in cnts]
...
def do_for_file(filepath):
    with_uid, courses = prepare_data(filepath)
    if len(with_uid) > 0:
        pass
    else:
        return
    ...
    for line in with_uid:
        ...
        # 注意， 此处不考虑第1个。
        for ii in range(1, min(len(courses), 11)):
            pipei_text = courses[sort_sims[ii][0]]
            print(pipei_text)

            # print(ii, get_uid(pipei_text, with_uid), pipei_text)  # 看下实际最相似的数据叫什么
            MCorrelation.add_relation(uid, get_uid(pipei_text, with_uid), 1, ii)


if __name__ == '__main__':
    inws = Path('./xx_title')
    for wfile in inws.rglob('*.md'):
        print('=' * 80)
        print(wfile.resolve())
        do_for_file(wfile)
